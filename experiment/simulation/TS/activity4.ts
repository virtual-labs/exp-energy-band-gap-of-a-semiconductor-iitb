...
var reading = 0;
var anim = false;
var heater_on_btn: Chemistry.Circle;
var heater_off_btn: Chemistry.Circle;
var temp1: HTMLInputElement;
var temp2: HTMLInputElement;
var temp3: HTMLInputElement;
var temp4: HTMLInputElement;
var Is1: HTMLInputElement;
var Is2: HTMLInputElement;
var Is3: HTMLInputElement;
var Is4: HTMLInputElement;
...
function activity4() {
	...
	pp.showtitle(
		`<p id="exp-title">Simulation with the Experimental Setup</p>`,
		3
	);
	canvas = pp.canvas;
	...
	let heater = new Chemistry.Rectangle(
		200,
		100,
		new Chemistry.Point(800, 770),
		canvas
	);
	heater.color = '#3399ff';

	let heater_text = new Chemistry.Geo_Text(
		'Heater Control',
		new Chemistry.Point(820, 840),
		canvas
	);
	heater_text.font = '23vw Arial';

	heater_on_btn = new Chemistry.Circle(
		new Chemistry.Point(850, 800),
		18,
		canvas
	);

	heater_off_btn = new Chemistry.Circle(
		new Chemistry.Point(950, 800),
		18,
		canvas
	);

	let img1 = new Chemistry.Custome_image(
		assembly,
	...
	let on = new Chemistry.Geo_Text(
		'On',
		new Chemistry.Point(838, 795),
		canvas
	);
	let off = new Chemistry.Geo_Text(
		'Off',
		new Chemistry.Point(938, 795),
		canvas
	);
...
function load_act4_options() {
	...
	for (let i = 0; i < energy_band.length; i++) {
		element.innerHTML += `<option value='${
			energy_band[i]['name']
		}'>${energy_band[i]['name'].toUpperCase()}</option>`;
	}
}
...
function least_count_select() {
	// a2_windowresize();
	pp.showdescription(
		`<p class='discription_text'>Select temp least count</p>`,
		3
	);
	show_panel(3);
	...
	let least_count: HTMLSelectElement = <HTMLSelectElement>(
		document.getElementById('least-count')
	);
...
function temp_dec() {
	...
	if (reading >= 2) pp.addtorightpannel(act4_first_btn, 3);
...
function getreadingtime() {
	...
	if (readingtime_temp >= 80) {
		pp.showdescription(
			`<p class='discription_text'>Turn Off heater</p>`,
			3
		);
		show_panel(3);
...
function a4_mouseclick_heater_on(e: MouseEvent) {
...
function a4_mouseclick_heater_off(e: MouseEvent) {
	let x = Math.round((e.clientX - rect.x) / lscale);
	let y = Math.round((canvas.height - (e.clientY - rect.y)) / lscale);
	console.log(x, y);

	let dec_btn: HTMLButtonElement = <HTMLButtonElement>(
		document.getElementById('temp_dec_btn')
	);
	...
	if (y >= 792 && y <= 814) {
		if (x >= 937 && x <= 959) {
			...
			pp.showdescription(
				`<p class='discription_text'>Note down the four readings for temperature and ammeter. <br> For more readings click on 'Temp. Dec' button</p>`,
				3
			);
			show_panel(3);
		}
...
function act4_verify() {
	for (let i = 0; i < 4; i++) {
		let tempEle: HTMLInputElement = <HTMLInputElement>(
			document.getElementById(`temp-inp-${i + 1}`)
		);
		let IsEle: HTMLInputElement = <HTMLInputElement>(
			document.getElementById(`Is-inp-${i + 1}`)
		);
